...
def magnetopause_flythrough(model_name, file_dir, trange, out_file,
                            variable_list=['B_x', 'B_y', 'B_z'], mms_probe=1):
    '''
    ...
    # fly trajectory through model output if not already created
    if not _isfile(out_file+'.nc'):
        results = S.ModelFlythrough(model_name, file_dir, variable_list,
                                    sat_time, sat_x, sat_y, sat_z, "GSM-car",
                                    output_name=out_file+'.nc',
                                    plot_coord='GSM')
    else:
        results = S.O.SF_read(out_file+'.nc')

    # functionalize the output and return
    kamodo_object = S.O.Functionalize_SFResults(model_name, results)
    return kamodo_object, mec_vars, fgm_vars, ion_vars, epoch, pos, sc_to_mp, \
        mp_dist, sc_dist, sza, results
